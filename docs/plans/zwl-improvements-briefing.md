...
> **Update 2026-09-22 — the "comes for free" framing above needs a
> correction.** Checked whether Walensee/PF-CH would actually get
> `StationsLinks` from the 4.3.0 bump, since the widget spec's acceptance
> scenario depends on it. **They don't, for the same reason Olten doesn't.**
> Read `flatland-rl` PR #441 directly: `stations_links` is computed inside
> `SparseRailGen._extract_stations_links()` — a method tied to that one
> generator's internal city/gate/pin bookkeeping, not a generic
> post-processing step. Then read `flatland-scenarios`'
> `ScenarioBuilder.to_rail_env()`: it builds envs via
> `rail_generator_from_grid_map()`, which wraps a fixed hand-authored grid and
> returns no `stations_links` at all, in any `flatland-rl` version — because
> `SparseRailGen` never runs. **Every real-topology scenario in this repo
> (Walensee, PF-CH, Olten) is built this way — none of them will ever get
> `StationsLinks` from a version bump.** Only a scenario freshly generated by
> `SparseRailGen` would. Full detail and the revised plan:
> `docs/plans/widget-b4-zug-weg-diagramm.md` §8.1. Consequence: the ported
> flatland-hmi dropdown is not on the critical path for Walensee (the
> priority scenario, §4) — building its axis directly from the scene JSON's
> own `stations` list is. The dropdown/full port stays right for a future
> `SparseRailGen`-based scenario, just not this one.
...
Recommendation (superseded by the update above for Walensee specifically):
Option A remains right *if/when* a scenario actually has `StationsLinks`. For
Walensee, v1 doesn't need a dropdown at all — one corridor, one axis, no
choice to present. Revisit A vs. B once a multi-link scenario is genuinely in
scope.
